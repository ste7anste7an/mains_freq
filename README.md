...
## Principle
The frequency meter consists of two microcontroller boards. A Raspberry Pico Pi and an ESP8266 board. The Pico Pi is responsible for measuring the actual mains frequency. The values read are passed to the ESP8266 using a serial port running at 9600 baud. The ESP8266 is programmed with a TLS version of the TASMOTA firmware. The analogue output of the ZMPT101B module is connected to the ADC0 convertor of the Pico Pi. 

The basic principle of the Mains Frequency meter is detetecting the zero crossings of the Alternate Voltage 50Hz signal.
The Pico Pi determines the raising zero crossings of the sine wave and counts 50 zero crossings. The total time for 50 zero crossings is measured using the internal micro seconds clock. From this measurement the mains frequency is derived. To smooth the output of the frequency measurements, a running average is taken using $freq = &alpha; freq_{meas} + (1-&alpha; ) freq$, with $freq_{meas}$ the raw measured value.

To prevent counting the zero crossing twice every 50Hz period, we only determine the zero crossing when a full period is detected. The 240V AC voltage is transformed down to a AC signal between 0 and 3 V. This signal is fed into the ADC of the Pico Pi. The average value $a_{off}$ is subtracted from the ADC reading. The parameers $a_{min}$ and $a_{max}$ are used to apply a hysteresis to the zero corossing detection. Now the time for detecting 50 periods $t_{50}$ is measured on a micro second scale. This time will be approximately 1 second. This is shown in de graph below
...
The frequency is adjusted by the parameter $us$ which is the number of microsecond counts in 1 second and is approximately $us=1000000$. A practical accurate value for $us$ is estimated to be 999960.
